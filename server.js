...
if (!process.env.MONGO_URL) {
  console.error("ОШИБКА: Не задан MONGO_URL в переменных окружения!");
}
...
const basicAuth = async (req, res, next) => {
  try {
    ...
    if (!nickname || !password)
      return res.status(401).json({ error: "Пустые данные входа" });
...
app.get("/api/users", async (req, res) => {
  try {
    ...
    const [users, total] = await Promise.all([
      User.find({}, "nickname firstName lastName createdAt")
        .skip(skip)
        .limit(limit)
        .lean(),
      User.countDocuments(),
...
app.put(
  ...
  [
    body("newPassword")
      .isLength({ min: 6 })
      .withMessage("Новый пароль слишком короткий"),
  ],
  async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty())
